...
export function initAssetsPage(): void {
    ...
    (window as any).selectNodeById = selectNodeById;
}
...
async function loadTree(): Promise<void> {
    try {
        ...
        if (container) container.innerHTML = '<div class="h-full flex flex-col items-center justify-center text-slate-400 opacity-50"><p>Error loading hierarchy</p></div>';
    }
}

export function selectNodeById(nodeId: string, skipPush = false): void {
    const row = document.querySelector(`.tree-row[data-id="${nodeId}"]`) as HTMLElement;
    if (row) {
        ...
        if (skipPush) row.dataset.skipPush = "true";
        row.click();
...
function createTreeNode(node: any, depth: number, path: any[] = []): HTMLElement {
    ...
    if (node.id === selectedNodeId) row.classList.add('active');
    ...
    if (node.children && node.children.length > 0) {
        ...
        toggle.onclick = (e) => {
            e.stopPropagation();
            const childContainer = wrapper.querySelector('.tree-children') as HTMLElement;
            if (childContainer) childContainer.classList.toggle('expanded');
...
function showNode(node: any, path: any[] = []): void {
    ...
    if (!emptyView || !contentView || !assetContent || !assetSidebar || !resizer) return;

    const row = document.querySelector(`.tree-row[data-id="${node.id}"]`) as HTMLElement;
