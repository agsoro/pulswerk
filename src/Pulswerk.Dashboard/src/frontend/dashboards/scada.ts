...
function svgColorFilter(hex: string): string {
...
export function renderBackgroundSvg(w: any): void {
    ...
    const rh = document.createElement('div'); rh.className = 'svg-resize'; el.appendChild(rh);

    el.addEventListener('click', e => {
        if (!isEditing || (e.target as HTMLElement).closest('.svg-toolbar')) return;
        e.stopPropagation();
...
function expandCanvasToFit(): void {
...
document.addEventListener('click', e => {
    if (!(e.target as HTMLElement).closest('.scada-svg-widget')) {
        document.querySelectorAll('.scada-svg-widget.selected').forEach(s => s.classList.remove('selected'));
    }
});

function setSvgZoom(wid: string, val: any): void {
    ...
    cfg.zoom = newZoom;
    const el = document.querySelector(`.scada-svg-widget[data-wid="${wid}"]`) as HTMLElement;
    if (el) {
        el.style.width = cfg.posW + '%';
        const inp = el.querySelector('.svg-zoom-input') as HTMLInputElement;
        if (inp) inp.value = String(newZoom);
    }
    expandCanvasToFit();
}

function initSvgWidgetDrag(el: HTMLElement, w: IWidget): void {
    const canvas = document.getElementById('dashCanvas')!;
    let startX: number, startY: number, startLeft: number, startTop: number, dragging = false;

    el.addEventListener('mousedown', e => {
        if (!isEditing || (e.target as HTMLElement).closest('.svg-toolbar') || (e.target as HTMLElement).closest('.svg-resize')) return;
    ...
    const rh = el.querySelector('.svg-resize') as HTMLElement;
    if (rh) rh.addEventListener('mousedown', e => {
        ...
        const onUp = () => {
            if (w.config) {
                ...
                const zoomInput = el.querySelector('.svg-zoom-input') as HTMLInputElement;
                if (zoomInput) zoomInput.value = String(w.config.zoom);
            }
            expandCanvasToFit();
            document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp);
        };
...
export function renderScadaPoint(w: any): void {
...
function getSvgContentBox(containerEl: HTMLElement): { left: number; top: number; width: number; height: number } {
...
function centerDotOffset(edge: string, w: number, h: number): { x: number; y: number } {
...
function positionScadaPoint(el: HTMLElement, cfg: IWidgetConfig): void {
    const anchorId = cfg.anchorSvg;
    if (anchorId) {
        const svgEl = document.querySelector(`.scada-svg-widget[data-wid="${anchorId}"]`) as HTMLElement;
        if (svgEl) {
...
function repositionAnchoredPoints(): void {
    if (!dashboard?.widgets) return;
    document.querySelectorAll('.scada-point[data-anchor-svg]').forEach(el => {
        const w = dashboard!.widgets!.find(x => x.id === (el as HTMLElement).dataset.wid);
        if (w && w.config?.anchorSvg) positionScadaPoint(el as HTMLElement, w.config);
    });
}

function findOverlappingSvg(pointEl: HTMLElement): HTMLElement | null {
    ...
    document.querySelectorAll('.scada-svg-widget').forEach(svgEl => {
        ...
        if (cx >= sr.left && cx <= sr.right && cy >= sr.top && cy <= sr.bottom) best = svgEl as HTMLElement;
    });
    return best;
}

function setPointEdge(wid: string, edge: string, dotEl: HTMLElement): void {
    const w = dashboard?.widgets?.find(x => x.id === wid); if (!w) return;
    const cfg = w.config || {}, parent = dotEl.closest('.scada-point') as HTMLElement; if (!parent) return;
    if (cfg.anchorEdge === edge && cfg.anchorSvg) {
        cfg.anchorEdge = ''; cfg.anchorSvg = ''; delete cfg.anchorW; delete cfg.anchorH; delete parent.dataset.anchorSvg;
    } else {
        const svgEl = findOverlappingSvg(parent); if (!svgEl) return;
        const svgWid = svgEl.dataset.wid; cfg.anchorSvg = svgWid; cfg.anchorEdge = edge; parent.dataset.anchorSvg = svgWid;
        const box = getSvgContentBox(svgEl);
        ...
        cfg.anchorW = box.width; cfg.anchorH = box.height;
    }
    w.config = cfg;
    parent.querySelectorAll('.sp-edge-dot').forEach(d => d.classList.toggle('active', (d as HTMLElement).dataset.edge === cfg.anchorEdge));
}

function updateEdgeDotVisibility(pointEl: HTMLElement): void {
    if (!isEditing) return;
    const overSvg = !!findOverlappingSvg(pointEl), anchored = !!pointEl.dataset.anchorSvg;
    pointEl.querySelectorAll('.sp-edge-dot').forEach(d => (d as HTMLElement).style.display = (overSvg || anchored) ? 'block' : 'none');
}

window.addEventListener('resize', () => { repositionAnchoredPoints(); });

async function updateScadaPointValues(w: IWidget): Promise<void> {
    const cfg = w.config || {}, keys = cfg.keys || []; if (!keys.length) return;
    let data: any; try { data = await DashboardService.fetchLatestValues(keys); } catch (e) { return; }
    renderScadaValues(w.id, keys, data);
    if (cfg.anchorSvg) {
        const el = document.querySelector(`.scada-point[data-wid="${w.id}"]`) as HTMLElement;
        if (el) requestAnimationFrame(() => positionScadaPoint(el, cfg));
    }
}

function renderScadaValues(wid: string, keys: string[], data: any): void {
    keys.forEach(key => {
        const el = document.querySelector(`.scada-point[data-wid="${wid}"] .sp-val[data-key="${key}"]`); if (!el) return;
...
async function updateAllScadaPoints(): Promise<void> {
    if (!dashboard?.widgets) return;
    ...
    let data: any; try { data = await DashboardService.fetchLatestValues(allScadaKeys); } catch (e) { return; }
    scadaWidgets.forEach(w => renderScadaValues(w.id, w.config?.keys || [], data));
}

function updateAllScadaPointsWithData(newData: Record<string, string>): void {
    if (!dashboard?.widgets) return;
...
async function showScadaPopup(key: string, triggerEl: HTMLElement): Promise<void> {
    ...
    const pp = meta.parentPath || [], pathHtml = pp.map((p: any, i: number) => `<a href="/plswk/Assets?node=${p.id}" style="color:inherit;text-decoration:none">${esc(p.name)}</a>${i < pp.length - 1 ? '<i class="fas fa-chevron-right" style="margin:0 0.4rem;font-size:0.55rem;opacity:0.4"></i>' : ''}`).join('');
    let currentVal = '---';
    try {
    ...
    const rect = triggerEl.getBoundingClientRect(); popup.style.left = Math.min(rect.right + 8, window.innerWidth - 360) + 'px'; popup.style.top = Math.max(rect.top - 20, 8) + 'px'; popup.style.display = 'block'; popup.classList.add('show');
    setTimeout(() => { const closer = (e: Event) => { if (!popup.contains(e.target as Node)) { hideScadaPopup(); document.removeEventListener('click', closer); } }; document.addEventListener('click', closer); }, 10);
}

function hideScadaPopup(): void { const popup = document.getElementById('scadaPopup'); if (popup) { popup.style.display = 'none'; popup.classList.remove('show'); } }

function initScadaPointDrag(el: HTMLElement, w: IWidget): void {
    let startX: number, startY: number, startPx: number, startPy: number, dragging = false; const canvas = document.getElementById('dashCanvas')!;
    el.addEventListener('mousedown', e => {
        if (!isEditing || (e.target as HTMLElement).closest('.sp-info') || (e.target as HTMLElement).closest('.sp-delete') || (e.target as HTMLElement).closest('.sp-edge-dot')) return;
        e.preventDefault(); dragging = true; el.classList.add('dragging'); startX = e.clientX; startY = e.clientY; startPx = el.offsetLeft; startPy = el.offsetTop;
        const onMove = (ev: MouseEvent) => { if (!dragging) return; el.style.left = (startPx + (ev.clientX - startX)) + 'px'; el.style.top = (startPy + (ev.clientY - startY)) + 'px'; };
        const onUp = () => {
            dragging = false; el.classList.remove('dragging');
            if (w) {
                ...
                if (anchorId) {
                    ...
                    if (svgEl) {
                        ...
                        cfg.posX = (el.offsetLeft + off.x - box.left) / box.width * 100; cfg.posY = (el.offsetTop + off.y - box.top) / box.height * 100; cfg.anchorW = box.width; cfg.anchorH = box.height;
                    }
                } else { cfg.posX = el.offsetLeft / canvas.clientWidth * 100; cfg.posY = el.offsetTop / canvas.clientHeight * 100; }
                w.config = cfg;
            }
            updateEdgeDotVisibility(el); document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp);
        };
        document.addEventListener('mousemove', onMove); document.addEventListener('mouseup', onUp);
    });
}

function handleSvgFileSelect(input: HTMLInputElement): void {
    if (!input.files?.length) return;
    const reader = new FileReader(); reader.onload = e => {
        pendingSvgContent = e.target?.result as string; const preview = document.getElementById('svgPreviewThumb'), label = document.getElementById('svgDropLabel');
        if (preview) { preview.innerHTML = pendingSvgContent; const svg = preview.querySelector('svg'); if (svg) { svg.style.width = '100%'; svg.style.height = 'auto'; svg.style.maxHeight = '160px'; } preview.classList.remove('hidden'); }
        if (label) label.classList.add('hidden');
    };
    reader.readAsText(input.files[0]);
}

let drawioCallback: any = null, drawioWindow: Window | null = null, _drawioPending = '', _drawioIsXml = false, _drawioExitAfterSave = false;
function openDrawioEditor(svgContent: string, onSave: (svg: string) => void): void {
    drawioCallback = onSave; _drawioPending = ''; _drawioIsXml = false;
    if (svgContent) {
        const trimmed = svgContent.trim();
        if (trimmed.startsWith('<mxfile') || trimmed.startsWith('<mxGraphModel')) { _drawioPending = svgContent; _drawioIsXml = true; }
        else if (trimmed.startsWith('<svg') && svgContent.includes('content=')) {
            try { const parser = new DOMParser(), doc = parser.parseFromString(svgContent, 'image/svg+xml'), svgEl = doc.querySelector('svg'), diagramXml = svgEl?.getAttribute('content'); if (diagramXml) { _drawioPending = diagramXml; _drawioIsXml = true; } else { _drawioPending = svgContent; } } catch (e) { _drawioPending = svgContent; }
        } else { _drawioPending = svgContent; }
    }
...
function closeDrawioEditor(): void { if (drawioWindow && !drawioWindow.closed) drawioWindow.close(); drawioWindow = null; drawioCallback = null; }

export function fitWidgetToSvg(w: any): void {
    const cfg = w.config; if (!cfg?.svg) return;
    try {
        const parser = new DOMParser(), doc = parser.parseFromString(cfg.svg, 'image/svg+xml'), svgEl = doc.querySelector('svg'); if (!svgEl) return;
        let svgW: number | undefined, svgH: number | undefined; const vb = svgEl.getAttribute('viewBox');
        if (vb) { const parts = vb.split(/[\s,]+/).map(Number); if (parts.length >= 4) { svgW = parts[2]; svgH = parts[3]; } }
        if (!svgW) svgW = parseFloat(svgEl.getAttribute('width') || '0') || 0; if (!svgH) svgH = parseFloat(svgEl.getAttribute('height') || '0') || 0;
        if (svgW > 0 && svgH > 0) {
            ...
            cfg.baseW = Math.round(svgW / containerW * 1000) / 10; cfg.baseH = Math.round(svgH / containerH * 1000) / 10;
            const zoom = cfg.zoom || 100; cfg.posW = Math.round(cfg.baseW * zoom / 100 * 10) / 10; cfg.posH = Math.round(cfg.baseH * zoom / 100 * 10) / 10;
        }
    } catch (e) { }
}

function editBackgroundSvg(wid: string): void {
    const bgWidget = dashboard!.widgets!.find(w => w.id === wid && w.type === 'background-svg'); if (!bgWidget) return;
    openDrawioEditor(bgWidget.config?.svg || '', newSvg => {
        bgWidget.config = bgWidget.config || {}; bgWidget.config.svg = newSvg; fitWidgetToSvg(bgWidget);
        const old = document.querySelector(`.scada-svg-widget[data-wid="${wid}"]`); if (old) old.remove(); renderBackgroundSvg(bgWidget);
    });
}

function createSvgInDrawio(): void {
    openDrawioEditor('', newSvg => {
        pendingSvgContent = newSvg;
        const preview = document.getElementById('svgPreviewThumb');
        if (preview) {
            preview.innerHTML = newSvg; const svg = preview.querySelector('svg');
            if (svg) { svg.style.width = '100%'; svg.style.height = 'auto'; svg.style.maxHeight = '160px'; const color = (document.getElementById('svgColor') as HTMLInputElement)?.value || '#38bdf8'; svg.style.filter = svgColorFilter(color); svg.style.opacity = '0.6'; }
...
window.addEventListener('message', function (evt: MessageEvent) {
    ...
    const target = drawioWindow && !drawioWindow.closed ? drawioWindow : null; if (!target) return;
    if (msg.event === 'init') {
        const content = _drawioPending || '';
        if (!content) target.postMessage(JSON.stringify({ action: 'load', xml: '<mxGraphModel><root><mxCell id="0"/><mxCell id="1" parent="0"/></root></mxGraphModel>', autosave: 0 }), '*');
        else if (_drawioIsXml) target.postMessage(JSON.stringify({ action: 'load', xml: content, autosave: 0 }), '*');
        else target.postMessage(JSON.stringify({ action: 'load', xml: '<mxGraphModel><root><mxCell id="0"/><mxCell id="1" parent="0"/></root></mxGraphModel>', autosave: 0 }), '*');
    }
        ...
        else target.postMessage(JSON.stringify({ action: 'status', message: 'Saved', modified: false }), '*');
    }
    else if (msg.event === 'exit') closeDrawioEditor();
});

function parseSvgElementIds(svgContent: string): string[] {
    ...
    try {
        ...
        doc.querySelectorAll('[id], [data-cell-id]').forEach(el => {
            ...
            if (!id) return;
            if (/^(cell|edge|mxCell|mxGraphModel|graph|foreignObject)\b/i.test(id)) return;
...
async function updateAllSvgAnimations(newData?: Record<string, string>): Promise<void> {
    if (window.ScadaAnimationController) await window.ScadaAnimationController.updateAll(newData);
}

function toggleIdPicker(wid: string): void {
    if (window.ScadaPickerController) window.ScadaPickerController.toggle(wid);
}

function closeIdPicker(): void {
    if (window.ScadaPickerController) window.ScadaPickerController.close();
}

function confirmRenameElement(): void {
    if (window.ScadaPickerController) window.ScadaPickerController.confirmRename();
}

function cancelRenameElement(): void {
    if (window.ScadaPickerController) window.ScadaPickerController.cancelRename();
}

function highlightPickerElement(idx: number, wid: string): void {
    if (window.ScadaPickerController) window.ScadaPickerController.highlightElement(idx, wid);
}

function unhighlightPickerElement(idx: number, wid: string): void {
    if (window.ScadaPickerController) window.ScadaPickerController.unhighlightElement(idx, wid);
}

function clickPickerElement(idx: number, wid: string): void {
    if (window.ScadaPickerController) window.ScadaPickerController.clickElement(idx, wid);
}

(window as any).svgColorFilter = svgColorFilter;
(window as any).renderBackgroundSvg = renderBackgroundSvg;
(window as any).expandCanvasToFit = expandCanvasToFit;
(window as any).setSvgZoom = setSvgZoom;
(window as any).initSvgWidgetDrag = initSvgWidgetDrag;
(window as any).renderScadaPoint = renderScadaPoint;
(window as any).getSvgContentBox = getSvgContentBox;
(window as any).centerDotOffset = centerDotOffset;
(window as any).positionScadaPoint = positionScadaPoint;
(window as any).repositionAnchoredPoints = repositionAnchoredPoints;
(window as any).findOverlappingSvg = findOverlappingSvg;
(window as any).setPointEdge = setPointEdge;
(window as any).updateEdgeDotVisibility = updateEdgeDotVisibility;
(window as any).updateScadaPointValues = updateScadaPointValues;
(window as any).renderScadaValues = renderScadaValues;
(window as any).updateAllScadaPoints = updateAllScadaPoints;
(window as any).updateAllScadaPointsWithData = updateAllScadaPointsWithData;
(window as any).showScadaPopup = showScadaPopup;
(window as any).hideScadaPopup = hideScadaPopup;
(window as any).initScadaPointDrag = initScadaPointDrag;
(window as any).handleSvgFileSelect = handleSvgFileSelect;
(window as any).openDrawioEditor = openDrawioEditor;
(window as any).closeDrawioEditor = closeDrawioEditor;
(window as any).fitWidgetToSvg = fitWidgetToSvg;
(window as any).editBackgroundSvg = editBackgroundSvg;
(window as any).createSvgInDrawio = createSvgInDrawio;
(window as any).parseSvgElementIds = parseSvgElementIds;
(window as any).updateAllSvgAnimations = updateAllSvgAnimations;
(window as any).toggleIdPicker = toggleIdPicker;
(window as any).closeIdPicker = closeIdPicker;
(window as any).confirmRenameElement = confirmRenameElement;
(window as any).cancelRenameElement = cancelRenameElement;
(window as any).highlightPickerElement = highlightPickerElement;
(window as any).unhighlightPickerElement = unhighlightPickerElement;
(window as any).clickPickerElement = clickPickerElement;
