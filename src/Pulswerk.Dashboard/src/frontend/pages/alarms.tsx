...
export function AlarmsPage() {
    ...
    return (
        <div class="flex flex-col gap-6 w-full page-enter">
            ...
            <div class="flex gap-2 overflow-x-auto no-scrollbar pb-1 mb-2 sm:flex-wrap" data-testid="alarm-filters">
                <button 
                    class={`filter-chip shrink-0 ${filter === '' ? 'active' : ''}`}
                    onClick={() => setFilter('')}
                >
                    All <span class="opacity-65 ml-1">{totalCount}</span>
                </button>
                <button 
                    class={`filter-chip chip-critical shrink-0 ${filter === 'CRITICAL' ? 'active' : ''}`}
                    onClick={() => setFilter('CRITICAL')}
                >
                    <i class="fas fa-skull-crossbones mr-1.5"></i> Critical <span class="opacity-65 ml-1">{countCritical}</span>
                </button>
                <button 
                    class={`filter-chip chip-major shrink-0 ${filter === 'MAJOR' ? 'active' : ''}`}
                    onClick={() => setFilter('MAJOR')}
                >
                    <i class="fas fa-exclamation-triangle mr-1.5"></i> Major <span class="opacity-65 ml-1">{countMajor}</span>
                </button>
                <button 
                    class={`filter-chip chip-minor shrink-0 ${filter === 'MINOR' ? 'active' : ''}`}
                    onClick={() => setFilter('MINOR')}
                >
                    <i class="fas fa-exclamation-circle mr-1.5"></i> Minor <span class="opacity-65 ml-1">{countMinor}</span>
                </button>
                <button 
                    class={`filter-chip chip-warning shrink-0 ${filter === 'WARNING' ? 'active' : ''}`}
                ...
                {countAcked > 0 && (
                    <button 
                        class={`filter-chip chip-acked shrink-0 ${filter === 'ACKED' ? 'active' : ''}`}
                ...
                {alarms.length === 0 ? (
                    ...
                    alarms.map((alarm: any) => {
                        ...
                        return (
                            <div 
                                key={alarm.alarmId}
                                class={`glass p-4 sm:p-5 rounded-2xl border border-slate-700 flex flex-col md:grid md:grid-cols-[auto_1fr_auto] items-start md:items-center gap-4 md:gap-6 transition-all duration-200 hover:bg-white/[0.02] ${
                                    isAcked ? 'opacity-70' : ''
                                }`}
                            >
                                <div class={`hidden md:block w-1.5 h-10 rounded ${
                                ...
                                <div class="flex flex-col gap-1 min-w-0 w-full">
                                    <div class="font-bold text-base text-slate-50 flex items-center gap-2 flex-wrap">
                                        <div class={`md:hidden w-2 h-2 rounded-full ${
                                            alarm.severity === 'CRITICAL' ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.7)]' : 
                                            alarm.severity === 'MAJOR' ? 'bg-amber-500' : 
                                            alarm.severity === 'MINOR' ? 'bg-sky-400' : 'bg-slate-400'
                                        }`}></div>
                                    ...
                                    <div class="text-xs text-slate-400 flex gap-x-4 gap-y-1 flex-wrap">
                                ...
                                <div class="w-full md:w-auto text-sm text-slate-400 text-left md:text-right shrink-0 pt-2 md:pt-0 border-t border-white/5 md:border-t-0 flex flex-row md:flex-col justify-between md:justify-start items-center md:items-end gap-2">
                                    <div class="font-mono text-xs">{new Date(alarm.time).toLocaleString()}</div>
                                    <div class="mt-0 md:mt-3 flex items-center justify-end gap-2">
                                        {!isAcked && alarm.alarmId && (
                                            <button 
                                                class={`min-h-[44px] md:min-h-0 btn-sm border border-emerald-500 hover:bg-emerald-500/10 text-emerald-500 rounded px-3 py-1.5 md:py-1 text-xs font-semibold inline-flex items-center gap-1.5 bg-transparent ${isWriteAllowed ? '' : 'hidden'}`}
                                        ...
                                        {alarm.telemetryKey && (
                                            <button 
                                                class="min-h-[44px] md:min-h-0 btn-sm border border-slate-600 text-slate-400 hover:text-slate-300 hover:bg-slate-700/50 rounded px-3 py-1.5 md:py-1 text-xs inline-flex items-center gap-1.5 bg-transparent"
                                        ...
                                        {alarm.alarmId && (
                                            <button 
                                                class={`min-h-[44px] md:min-h-0 btn-sm border border-slate-600 text-slate-400 hover:text-red-400 hover:border-red-400/30 hover:bg-red-500/5 rounded px-3 py-1.5 md:py-1 text-xs inline-flex items-center gap-1.5 bg-transparent ${isWriteAllowed ? '' : 'hidden'}`}
            ...
            {ackAlarm && (
                <div class="fixed inset-0 z-[9000] flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm p-0 sm:p-4" data-testid="ack-modal" onClick={() => !submittingAck && setAckAlarm(null)}>
                    <div class="glass w-full sm:w-[min(480px,90vw)] rounded-t-3xl sm:rounded-xl p-6 sm:p-8 flex flex-col gap-5 relative border border-white/10 shadow-2xl animate-slide-up-mobile sm:animate-none" onClick={e => e.stopPropagation()}>
                        <div class="flex justify-between items-center">
                            ...
                            <button 
                                ...
                                class="bg-transparent border-0 text-slate-400 cursor-pointer text-xl hover:text-white p-2"
                        ...
                        <div class="flex gap-3 justify-end mt-2">
                            <button 
                                ...
                                class="min-h-[44px] bg-transparent border border-slate-700 text-slate-400 px-5 py-2 rounded-lg cursor-pointer hover:bg-white/5 transition-colors text-sm font-semibold"
                                disabled={submittingAck}
                            >
                                Cancel
                            </button>
                            <button 
                                ...
                                class="min-h-[44px] bg-emerald-500 hover:bg-emerald-600 border-0 text-white font-semibold px-6 py-2 rounded-lg cursor-pointer transition-colors text-sm shadow-lg shadow-emerald-500/10"
                                disabled={submittingAck}
                            >
