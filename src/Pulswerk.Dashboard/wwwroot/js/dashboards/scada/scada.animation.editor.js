...
export function initAnimRuleEditor(rules) {
    currentRules = JSON.parse(JSON.stringify(rules || [])).map((r) => {
        if (r.telemetryKey && !r.telemetryKeys)
            r.telemetryKeys = [r.telemetryKey];
        return r;
    });
    renderAnimRules();
}
...
export function addAnimRuleTelemetryKey(idx, key) {
    if (!key || !key.trim())
        return;
...
export function removeAnimRuleTelemetryKey(idx, key) {
    const current = currentRules[idx].telemetryKeys || [];
...
export function renderAnimRules() {
                        ...
                        <div style="display:flex; flex-wrap:wrap; gap:0.25rem; min-height: 28px; padding:0.25rem; background:rgba(0,0,0,0.2); border:1px solid #334155; border-radius:0.3rem; align-items:center;">
                            ${(rule.telemetryKeys || []).map((key, kIdx) => `
                                <span style="background:#0f172a; color:#e2e8f0; font-size:0.65rem; padding:0.1rem 0.4rem; border-radius:0.2rem; display:inline-flex; align-items:center; gap:0.3rem;" title="Use v${kIdx} in formula">
                                    ...
                                    <i class="fas fa-times" style="cursor:pointer; color:#ef4444;" onclick="event.preventDefault();removeAnimRuleTelemetryKey(${idx}, '${window.esc ? window.esc(key) : key}')"></i>
                                </span>
                            `).join('')}
