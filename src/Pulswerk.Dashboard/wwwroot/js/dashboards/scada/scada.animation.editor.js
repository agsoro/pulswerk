...
let currentPreviewRuleIdx = null;
export function initAnimRuleEditor(rules) {
    currentRules = JSON.parse(JSON.stringify(rules || [])).map((r) => {
        if (r.dataKey && !r.dataKeys)
            r.dataKeys = [r.dataKey];
        return r;
    });
    renderAnimRules();
}
export function getAnimRules() {
    return currentRules;
}
export function addAnimRule() {
    currentRules.push({
        dataKeys: [],
        elementId: '',
        formula: '',
...
export function removeAnimRule(idx) {
    currentRules.splice(idx, 1);
    renderAnimRules();
}
export function updateAnimRule(idx, field, value) {
    if (idx >= 0 && idx < currentRules.length) {
        currentRules[idx][field] = value;
...
export function addAnimRuleDataKey(idx, key) {
    if (!key || !key.trim())
        return;
...
function renderAnimRules() {
    ...
    container.innerHTML = currentRules.map((rule, idx) => {
        return `
                        ...
                        <div style="display:flex; flex-wrap:wrap; gap:0.25rem; min-height: 28px; padding:0.25rem; background:rgba(0,0,0,0.2); border:1px solid #334155; border-radius:0.3rem; align-items:center;">
                            ${(rule.dataKeys || []).map((key, kIdx) => `
                                <span style="background:#0f172a; color:#e2e8f0; font-size:0.65rem; padding:0.1rem 0.4rem; border-radius:0.2rem; display:inline-flex; align-items:center; gap:0.3rem;" title="Use v${kIdx} in formula">
                                    <span style="opacity:0.5;margin-right:2px;">v${kIdx}:</span>${window.esc ? window.esc(key) : key}
                                    <i class="fas fa-times" style="cursor:pointer; color:#ef4444;" onclick="event.preventDefault();removeAnimRuleDataKey(${idx}, '${window.esc ? window.esc(key) : key}')"></i>
                                </span>
                            `).join('')}
                            ...
                            <div style="position:relative; display:inline-block;">
                                ...
                                <div style="display:none; position:absolute; top:100%; left:0; z-index:100; margin-top:0.2rem; background:#1e293b; border:1px solid #334155; border-radius:0.3rem; padding:0.3rem; width:200px; box-shadow:0 10px 15px -3px rgba(0,0,0,0.5);">
                                    ...
                                    <datalist id="dl_keys_${idx}">
                                        ${availableKeys.filter(k => !(rule.dataKeys || []).includes(k)).map(k => `<option value="${window.esc ? window.esc(k) : k}">`).join('')}
                                    </datalist>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style="display:flex; flex-wrap:wrap; gap:0.25rem; min-height: 28px; padding:0.25rem; background:rgba(0,0,0,0.2); border:1px solid #334155; border-radius:0.3rem; align-items:center;">
                            ${(rule.elementId || '').split(',').map(s => s.trim()).filter(Boolean).map(id => `
                                <span style="background:#0f172a; color:#e2e8f0; font-size:0.65rem; padding:0.1rem 0.4rem; border-radius:0.2rem; display:inline-flex; align-items:center; gap:0.3rem;">
                                    ${window.esc ? window.esc(id) : id}
                                    <i class="fas fa-times" style="cursor:pointer; color:#ef4444;" onclick="event.preventDefault();removeAnimRuleElementId(${idx}, '${window.esc ? window.esc(id) : id}')"></i>
                                </span>
                            `).join('')}
            ...
            .map(id => `<option value="${window.esc ? window.esc(id) : id}">`)
            .join('')}
                                    </datalist>
                                </div>
                            </div>
                ...
                <div style="margin-bottom:0.5rem;">
                    ...
                    <input type="text" class="form-input" style="padding:0.25rem 0.5rem; font-size:0.75rem;" value="${window.esc ? window.esc(rule.formula || '') : rule.formula}" onchange="updateAnimRule(${idx}, 'formula', this.value)" placeholder="JS expression using v0, v1...">
                </div>
                ...
                <div>
                    ...
                    <input type="text" class="form-input" style="padding:0.25rem 0.5rem; font-size:0.75rem;" value="${window.esc ? window.esc((rule.styles || []).join(', ')) : (rule.styles || []).join(', ')}" onchange="updateAnimRule(${idx}, 'styles', this.value)">
                </div>
            </div>
        `;
    }).join('');
}
export function openAnimPreview(ruleIdx) {
        ...
        if (svgEl) {
            ...
            if (select) {
                ...
                select.innerHTML = '<option value="">— Select Element —</option>' +
                    Array.from(ids).map(id => `<option value="${window.esc ? window.esc(id) : id}">${window.esc ? window.esc(id) : id}</option>`).join('');
            }
        }
    }
    else {
...
export function closeAnimPreview() {
    const popup = document.getElementById('scadaAnimPreview');
    if (popup)
...
export function onPreviewElementChange() {
    const select = document.getElementById('previewElementSelect');
    if (!select)
        return;
    const popup = document.getElementById('scadaAnimPreview');
...
export function clearPreviewClasses() {
    if (currentPreviewElement) {
        ScadaAnimationController.allClasses.forEach(c => currentPreviewElement.classList.remove(c));
        renderAnimChips(document.getElementById('previewElementSelect')?.value || '');
...
export function togglePreviewClass(cls) {
    if (!currentPreviewElement)
        return;
    ...
    if (currentPreviewElement.classList.contains(cls)) {
        ...
        if (category) {
            ...
            category.classes.forEach(c => {
                if (c !== cls)
                    currentPreviewElement.classList.remove(c);
            });
        }
            ...
            if (hasDots) {
                ...
                if (pathEl) {
                    ...
                    if (!existing || existing.reverse !== reverse || existing.fast !== fast || existing.color !== colorClass) {
                        ...
                        ScadaAnimationController.spawnDots(key, svgEl, pathEl, reverse, fast, colorClass);
                        if (!ScadaAnimationController.dotRafId) {
...
function renderAnimChips(elementId) {
