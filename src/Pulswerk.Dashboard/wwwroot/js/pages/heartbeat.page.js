...
let lastPush = 0;
let lastPull = 0;
export function initHeartbeatPage(initialStats) {
...
async function updateStats() {
    try {
        ...
        if (mem)
            mem.textContent = `${data.workingSetMb} MB / ${data.gcHeapMb} MB`;
        const tcpEl = document.getElementById('tcpConns');
        if (tcpEl)
            tcpEl.textContent = data.tcpConnections;
        // Oldest device seen
        const oldest = document.getElementById('oldestSeen');
        if (oldest && data.oldestDeviceSeenUtc)
            oldest.textContent = data.oldestDeviceSeenUtc + ' UTC';
        // Format uptime
        ...
        pushBuffer.shift();
        pushBuffer.push(pushDiff);
        pullBuffer.shift();
        pullBuffer.push(pullDiff);
        if (chart) {
            chart.updateSeries([
                { data: pushBuffer },
                { data: pullBuffer }
...
window.initHeartbeatPage = initHeartbeatPage;
