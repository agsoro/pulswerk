export class DrawioCodec {
    ...
    static async compress(xmlStr) {
        try {
            ...
            for (let i = 0; i < compressedArray.length; i++)
                binaryStr += String.fromCharCode(compressedArray[i]);
    ...
    static updateXmlContent(xml, oldId, newId) {
    ...
    static async renameCellId(contentXml, oldId, newId) {
        try {
            if (!contentXml || typeof contentXml !== 'string')
                return contentXml;
