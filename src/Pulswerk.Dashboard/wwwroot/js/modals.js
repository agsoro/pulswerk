...
let scheduleValueType = 'real';
let scheduleStates = null;
...
function switchTelemetryTab(tabName) {
    ...
    if (schedContent)
        schedContent.style.display = 'none';
...
async function openTelemetryDetails(key) {
    ...
    try {
        ...
        if (telTitle)
            telTitle.textContent = (fn ? fn(key) : null) || meta.name || key;
        const telMeta = document.getElementById('telMeta');
        if (telMeta)
            telMeta.textContent = meta.fullName || key;
        const telUnitLabel = document.getElementById('telUnitLabel');
        if (telUnitLabel)
            telUnitLabel.textContent = meta.units || '';
        renderModalBreadcrumb('telPath', path);
        // Setup Favorite Star
        const favBtn = document.getElementById('telFavBtn');
        if (favBtn) {
            if (window.pwCanEditFavorites) {
                favBtn.style.display = 'inline-flex';
                const isFav = window.pw_fav?.get('deziko_favorites')?.includes(key);
                ...
                favBtn.onclick = () => {
                    if (typeof window.toggleFavorite === 'function') {
                        ...
                        const nowFav = window.pw_fav?.get('deziko_favorites')?.includes(key);
        ...
        if (stepperGroup)
            stepperGroup.style.display = 'none';
        ...
        try {
            ...
            if (raw != null) {
                ...
                if (telLiveValue)
                    telLiveValue.textContent = currentVal;
            }
        }
        catch (e) {
            console.error("DEBUG: fetchLatestValues failed", e);
        }
        ...
        if (meta.isWritable && !isScheduleObj) {
            if (enums && Object.keys(enums).length > 0) {
                ...
                if (editInput) {
                    ...
                    editInput.step = window.isTemperatureUnit(meta.units) ? '0.5' : '1';
                }
                ...
                if (stepperGroup)
                    stepperGroup.style.display = 'flex';
            }
        }
...
function closeTelemetryDetails() {
    stopHistoryRefresh();
    const detailsModal = document.getElementById('telemetryDetailsModal');
    if (detailsModal)
        detailsModal.style.display = 'none';
    ...
    const telTitle = document.getElementById('telTitle');
    if (telTitle)
        telTitle.textContent = 'Telemetry Details';
    const telUnitLabel = document.getElementById('telUnitLabel');
    if (telUnitLabel)
        telUnitLabel.textContent = '';
    const telMeta = document.getElementById('telMeta');
    if (telMeta)
        telMeta.textContent = '';
    const telLiveValue = document.getElementById('telLiveValue');
    ...
    if (propsTable)
        propsTable.classList.add('hidden');
    ...
    const searchInput = document.getElementById('telSidebarSearch');
    if (searchInput)
        searchInput.value = '';
...
function renderTelemetrySidebarList(activeKey) {
    const listContainer = document.getElementById('telSidebarList');
    if (!listContainer)
        return;
    ...
    sortedKeys.forEach(k => {
        ...
        const isFav = window.pw_fav?.get('deziko_favorites')?.includes(k.key);
...
function filterTelemetrySidebar() {
    const searchInput = document.getElementById('telSidebarSearch');
    if (!searchInput)
        return;
...
async function loadPropsForDetails(key) {
...
async function refreshHistoryData() {
    if (!currentHistoryKey)
        return;
    if (document.getElementById('telemetryDetailsModal')?.style.display !== 'flex')
        return;
    const range = historyTw ? historyTw.getRange() : { startTs: Date.now() - 3600000, endTs: Date.now(), mode: 'realtime' };
...
function step(n) {
    ...
    if (currentEditKey) {
        ...
        if (window.isTemperatureUnit(meta.units)) {
            stepVal = 0.5;
        }
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (result.success) {
                ...
                const telLiveValue = document.getElementById('telLiveValue');
                if (telLiveValue)
                    telLiveValue.textContent = formatted;
...
async function loadScheduleForDetails(key, props) {
    ...
    try {
        ...
        if (statesProp) {
            try {
                statesProp.value && (scheduleStates = JSON.parse(statesProp.value));
            }
            catch (e) { }
        }
...
function toggleScheduleEdit(edit) {
    ...
    if (saveBtn) {
        ...
        saveBtn.innerHTML = '<i class="fas fa-save mr-1"></i> <span>Save</span>';
    }
    renderSchedule();
}
...
// --- Compatibility Shims ---
window.openHistory = (key) => openTelemetryDetails(key);
window.openEdit = (key) => openTelemetryDetails(key);
window.openProperties = (key) => openTelemetryDetails(key);
window.openScheduleView = (key) => openTelemetryDetails(key);
window.openTelemetryDetails = openTelemetryDetails;
window.closeTelemetryDetails = closeTelemetryDetails;
window.closeHistory = closeTelemetryDetails;
window.closeEdit = closeTelemetryDetails;
window.closeProperties = closeTelemetryDetails;
window.closeSchedule = closeTelemetryDetails;
...
window.startTelemetryEdit = startTelemetryEdit;
window.cancelTelemetryEdit = cancelTelemetryEdit;
...
window.filterTelemetrySidebar = filterTelemetrySidebar;
