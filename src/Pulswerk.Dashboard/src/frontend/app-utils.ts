...
const PulswerkValue = (() => {
     ...
     */
    function isMultiState(type: string): boolean {
        if (!type) return false;
     ...
     */
    function formatDisplay(val: any, type: string, decimals: number = 2): string {
        // Null / missing → placeholder
        if (val === null || val === undefined || val === '---' || val === '') return '---';

        // Schedule → label only (rendered separately)
        if (isSchedule(type)) return 'Schedule';
        ...
        return num.toLocaleString(
            typeof currentLang !== 'undefined' ? currentLang : undefined,
     ...
     */
    function parseDisplay(displayVal: string, type: string): number {
        ...
        if (isBinary(type)) {
            ...
            if (['on', '1', 'true', 'active', 'ein', 'ja', 'yes'].includes(lower)) return 1;
            if (['off', '0', 'false', 'inactive', 'aus', 'nein', 'no'].includes(lower)) return 0;
    ...
    function valueClass(type: string): string {
        if (isBinary(type)) return 'pw-val-binary';
        if (isMultiState(type)) return 'pw-val-multistate';
        if (isSchedule(type)) return 'pw-val-schedule';
 ...
 */
function getPointIcon(type: string): string {
 ...
 */
function toggleFavorite(key: string, btn?: HTMLElement): void {
    ...
    if (index > -1) {
        ...
        if (btn) {
            ...
            if (iEl) iEl.className = 'far fa-star';
        ...
        if (btn) {
            const iEl = btn.querySelector('i');
            if (iEl) iEl.className = 'fas fa-star text-amber-400';
 ...
 */
function updateStarState(key: string, btn?: HTMLElement): void {
    if (!btn) return;
...
(window as any).PulswerkValue = PulswerkValue;
(window as any).formatNumber = formatNumber;
(window as any).getPointIcon = getPointIcon;
(window as any).toggleFavorite = toggleFavorite;
(window as any).updateStarState = updateStarState;
