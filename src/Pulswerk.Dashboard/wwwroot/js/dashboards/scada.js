...
function setPointEdge(wid, edge, dotEl) {
    const w = dashboard?.widgets?.find(x => x.id === wid); if (!w) return;
    const cfg = w.config || {}, parent = dotEl.closest('.scada-point'); if (!parent) return;
...
async function updateScadaPointValues(w) {
    ...
    let data; try { data = await api(`LatestValues&keys=${keys.join(',')}`); } catch (e) { return; }
...
function renderScadaValues(wid, keys, data) {
    keys.forEach(key => {
        ...
        const val = data?.[key] || '---', meta = allKeys.find(k => k.key === key) || {}, type = meta.type || '', display = PulswerkValue.formatDisplay(val, type);
...
async function updateAllScadaPoints() {
    ...
    let data; try { data = await api(`LatestValues&keys=${allScadaKeys.join(',')}`); } catch (e) { return; }
...
async function showScadaPopup(key, triggerEl) {
    ...
    const meta = allKeys.find(k => k.key === key) || {}, icon = typeof getPointIcon === 'function' ? getPointIcon(meta.type || '') : '<i class="fas fa-microchip"></i>', keyJs = key.replace(/'/g, "\\'");
    ...
    setTimeout(() => { const closer = e => { if (!popup.contains(e.target)) { hideScadaPopup(); document.removeEventListener('click', closer); } }; document.addEventListener('click', closer); }, 10);
}

function hideScadaPopup() { const popup = document.getElementById('scadaPopup'); if (popup) { popup.style.display = 'none'; popup.classList.remove('show'); } }

function initScadaPointDrag(el, w) {
    let startX, startY, startPx, startPy, dragging = false; const canvas = document.getElementById('dashCanvas');
    el.addEventListener('mousedown', e => {
        ...
        const onMove = ev => { if (!dragging) return; el.style.left = (startPx + (ev.clientX - startX)) + 'px'; el.style.top = (startPy + (ev.clientY - startY)) + 'px'; };
        const onUp = () => {
            dragging = false; el.classList.remove('dragging');
            if (w) {
...
function handleSvgFileSelect(input) {
    ...
    const reader = new FileReader(); reader.onload = e => {
        pendingSvgContent = e.target.result; const preview = document.getElementById('svgPreviewThumb'), label = document.getElementById('svgDropLabel');
...
function closeDrawioEditor() { if (drawioWindow && !drawioWindow.closed) drawioWindow.close(); drawioWindow = null; drawioCallback = null; }
...
function fitWidgetToSvg(w) {
    ...
    try {
        ...
        let svgW, svgH; const vb = svgEl.getAttribute('viewBox');
        if (vb) { const parts = vb.split(/[\s,]+/).map(Number); if (parts.length >= 4) { svgW = parts[2]; svgH = parts[3]; } }
        if (!svgW) svgW = parseFloat(svgEl.getAttribute('width')) || 0; if (!svgH) svgH = parseFloat(svgEl.getAttribute('height')) || 0;
        if (svgW > 0 && svgH > 0) {
...
function createSvgInDrawio() {
    openDrawioEditor('', newSvg => {
        ...
        if (preview) {
            ...
            if (svg) { svg.style.width = '100%'; svg.style.height = 'auto'; svg.style.maxHeight = '160px'; const color = document.getElementById('svgColor')?.value || '#38bdf8'; svg.style.filter = svgColorFilter(color); svg.style.opacity = '0.6'; }
