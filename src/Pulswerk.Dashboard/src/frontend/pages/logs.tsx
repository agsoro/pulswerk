...
export function LogsPage() {
    ...
    const [level, setLevel] = useState<string>(() => localStorage.getItem('logLevelPref') || 'all');
    ...
    return (
        <div class="flex flex-col gap-5 h-[calc(100vh-110px)] w-full page-enter">
            {/* Toolbar */}
            <div class="flex justify-between items-center bg-slate-800/40 backdrop-blur-md border border-white/5 rounded-xl px-5 py-3 shrink-0">
                <div class="flex gap-2 items-center">
                    ...
                    <select 
                        ...
                        class="ml-4 bg-slate-900 border border-slate-700 rounded px-2.5 py-1 text-xs font-semibold text-slate-300 focus:outline-none focus:border-sky-500 cursor-pointer"
                    >
                ...
                <div class="flex items-center gap-4 text-slate-400 text-xs select-none">
                    ...
                    <button 
                        onClick={fetchLogs} 
                        class="text-sky-400 hover:text-sky-300 font-bold bg-transparent border-0 p-0 cursor-pointer inline-flex items-center gap-1"
                    >
                        <i class="fas fa-sync-alt text-[0.7rem]"></i> {t('btn_refresh')}
                    </button>
                </div>
            </div>
            ...
            <div 
                ref={logContainerRef}
                class="flex-1 glass rounded-2xl border border-slate-700/60 p-5 font-mono text-[0.78rem] overflow-y-auto flex flex-col gap-1.5 shadow-2xl custom-scrollbar" 
                style={{ backgroundColor: '#020617' }}
                data-testid="log-container"
            >
                {logs.length === 0 ? (
                    <div class="h-full flex items-center justify-center text-slate-500 italic">
                        No log records found for severity "{level}"
                    </div>
                ) : (
                    logs.map((log, idx) => {
