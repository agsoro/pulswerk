...
function renderAnimChips(elementId: string): void {
    ...
    if (currentPreviewElement) {
        ...
        if (['path', 'line', 'polyline'].includes(tag)) hasPath = true;
        else if (currentPreviewElement.querySelector('path, line, polyline')) hasPath = true;
    }

    let html = '';
    animCategories.forEach(cat => {
        if (cat.name.startsWith('Particle') && !hasPath) return;
