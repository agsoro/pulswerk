...
export class ScadaAnimationController {
    ...
    static applyAnimationRules(svgWidgetEl: HTMLElement | null, rules: any[], data: any): void {
        if (!rules?.length || !svgWidgetEl) return;
        ...
        for (const rule of rules) {
            ...
            elementIds.forEach((id: string) => {
                const target = svg.querySelector('#' + (window as any).CSS.escape(id)) || 
                               svg.querySelector(`[data-cell-id="${(window as any).CSS.escape(id)}"]`);
                if (!target) return;
                ...
                if (dataKeys.length > 0 && values.every((v: any) => v == null)) return;
                
                if (!rule.formula || ScadaAnimationController.evaluateCondition(values, rule.formula)) {
                    const styles = rule.styles || [];
                    styles.forEach((cls: string) => { 
                        if (ScadaAnimationController.allClasses.includes(cls)) target.classList.add(cls); 
                    });
                }
            });
        }
    ...
    static updateDotAnimations(): void {
        document.querySelectorAll('.scada-svg-widget').forEach(widget => {
            ...
            svg.querySelectorAll('[id], [data-cell-id]').forEach(el => {
                ...
                if (!elId) return;
                const key = (widget as HTMLElement).dataset.wid + ':' + elId;
                if (hasDots) {
                    ...
                    if (existing && existing.reverse === reverse && existing.fast === fast && existing.color === colorClass) return;
                    if (existing) ScadaAnimationController.cleanupDots(key);
                    ScadaAnimationController.spawnDots(key, svg, pathEl as SVGGeometryElement, reverse, fast, colorClass);
    ...
    static spawnDots(key: string, _svg: SVGElement, pathEl: SVGGeometryElement, reverse: boolean, fast: boolean, colorClass: string): void {
        let totalLen: number;
        try {
            ...
            } else totalLen = (pathEl as SVGPathElement).getTotalLength();
        } catch (e) { return; }
        ...
        for (let i = 0; i < DOT_COUNT; i++) {
            ...
            circle.setAttribute('r', String(DOT_RADIUS)); circle.classList.add('scada-dot');
            ...
            anim.dots.forEach((dot: any) => {
                ...
                try {
                    ...
                    if (anim.pathEl.tagName === 'line') {
                        ...
                        const t = dot.offset / dot.totalLen; pt = { x: x1 + (x2 - x1) * t, y: y1 + (y2 - y1) * t };
                    } else pt = anim.pathEl.getPointAtLength(dot.offset);
                    dot.el.setAttribute('cx', String(pt.x)); dot.el.setAttribute('cy', String(pt.y));
    ...
    static cleanupDots(key: string): void {
