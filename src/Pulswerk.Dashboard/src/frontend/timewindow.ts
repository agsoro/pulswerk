...
function createTimeWindowSelector(container: HTMLElement, opts: any = {}): ITimeWindowSelector {
    let mode: string = opts.mode || 'realtime';
    let realtimeMs: number = opts.realtimeMs || 3600000;
    let histFrom: number | null = opts.histFrom || null;
    let histTo: number | null = opts.histTo || null;
    ...
    // Cache DOM refs
    const labelEl = wrapper.querySelector('.tw-label') as HTMLElement;
    const iconEl = wrapper.querySelector('.tw-mode-icon') as HTMLElement;
    const dropdown = wrapper.querySelector('.tw-dropdown') as HTMLElement;
    const presetGrid = wrapper.querySelector('.tw-presets-grid') as HTMLElement;
    const realtimePanel = wrapper.querySelector('.tw-realtime-panel') as HTMLElement;
    const histPanel = wrapper.querySelector('.tw-history-panel') as HTMLElement;
    const tabs = wrapper.querySelectorAll('.tw-tab');
    const histFromEl = wrapper.querySelector('.tw-hist-from') as HTMLInputElement;
    const histToEl = wrapper.querySelector('.tw-hist-to') as HTMLInputElement;
    const applyBtn = wrapper.querySelector('.tw-apply-btn') as HTMLElement;

    // Build presets
    function buildPresets() {
        presetGrid.innerHTML = TW_PRESETS.map(p =>
            `<button class="tw-preset${p.ms === realtimeMs ? ' active' : ''}" data-ms="${p.ms}">${p.label}</button>`
        ).join('');
    }
    buildPresets();
    ...
    wrapper.addEventListener('click', e => {
        if ((e.target as HTMLElement).closest('.tw-dropdown')) return;
        e.stopPropagation();
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            const m = (tab as HTMLElement).dataset.twMode;
            if (!m) return;
            mode = m;
            tabs.forEach(t => t.classList.toggle('active', (t as HTMLElement).dataset.twMode === m));
    ...
    presetGrid.addEventListener('click', e => {
        const btn = (e.target as HTMLElement).closest('.tw-preset') as HTMLElement;
        if (!btn) return;
    ...
    // Public API
    function getRange() {
        if (mode === 'history' && histFrom && histTo) return { startTs: histFrom, endTs: histTo, mode: 'history' };
        const now = Date.now();
        return { startTs: now - realtimeMs, endTs: now, mode: 'realtime', realtimeMs };
    }

    function setPreset(ms: number) {
        realtimeMs = ms; mode = 'realtime';
        buildPresets(); updateLabel();
        onChange(getRange());
    }
...
(window as any).createTimeWindowSelector = createTimeWindowSelector;
